 ...
 * Shell matches the landing map atmosphere with a centered auth card.
 */
...
/** Avoid UnlockProvider (Gate can mount pre-launch outside it). */
function useReduceMotionFlag() {
  const [reduceMotion] = useState(() => {
    try {
      if (localStorage.getItem('ty-motion') === 'true') return true;
    } catch { /* ignore */ }
...
const Gate: React.FC = () => {
  ...
  return (
    <main className="relative min-h-screen flex flex-col bg-background grain-overlay overflow-hidden">
      ...
      <div
        className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 py-10 overflow-y-auto overscroll-contain"
        style={{
      ...
      >
        <p className="mb-4 font-display text-[11px] uppercase tracking-[0.28em] text-cream/70">
          enter the list
        </p>
